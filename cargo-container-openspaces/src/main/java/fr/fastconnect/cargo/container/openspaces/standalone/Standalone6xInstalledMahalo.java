...
/**
 * Implementation of {@link org.codehaus.cargo.container.InstalledLocalContainer} that starts a Mahalo transaction manager for OpenSpaces.
 * This is using GigaSpaces service-grid starter classes.
 */
public class Standalone6xInstalledMahalo extends AbstractInstalledLocalContainer {
	private final static Log logger = LogFactory.getLog(Standalone6xInstalledMahalo.class);
	
	private final static long TM_START_WAIT_TIME=5000;
	
	private AntContainerExecutorThread puInstanceRunner;
	
	public Standalone6xInstalledMahalo(final LocalConfiguration configuration) {
		super(configuration);
	}
	
	public String getId() {
		return "standalone-installed-mahalo-6x";
	}
	
	public String getName() {
		return "Standalone Installed Jini distributed transaction service 6.x";
	}
	...
	protected void doStart(final Java java) throws Exception {
		logger.info("Prepare to start Mahalo");
		...
		java.createArg().setValue("com.gigaspaces.start.services=\"TM\"");
		
		puInstanceRunner = new AntContainerExecutorThread(java);
		
		logger.info("Starting Mahalo...");
	...
	protected void waitForCompletion(final boolean waitForStarting) throws InterruptedException {
		// Assume I need 10 seconds to start gigaspaces container.
		Thread.sleep(Standalone6xInstalledMahalo.TM_START_WAIT_TIME);
		logger.info("Mahalo has started");
	}

	@Override
	protected void doStop(final Java java) throws Exception {
		logger.info("stopping Mahalo");
